...
export default function Business({ route }) {
    ...
    const Business = cityData[selectedCity.name].Businesses;
    

    const filterBussiness = (Business) => {
        return Business.name.toLowerCase().includes(searchQuery.toLowerCase());
    };

    return (
        <ImageBackground source={require('./Business.jpeg')} >
        <View style={{backgroundColor:"transparent"}}>
            ...
            <FlatList
                data={Business.filter(filterBussiness)}
